...
const EP0_RING_SIZE: usize = 8;
const DOORBELL_BASE: u32 = 0x400;
const XHCI_CC_SUCCESS: u8 = 1;

struct Ep0Ring {
    trbs: [Trb; EP0_RING_SIZE],
    enqueue: usize,
    cycle: u32,
}

static mut EP0_RING: Ep0Ring = Ep0Ring {
    trbs: [Trb::zero(); EP0_RING_SIZE],
    enqueue: 0,
    cycle: 1,
};

/// 向 EP0 传输环写入 TRB
unsafe fn ep0_enqueue(trb: Trb) -> bool {
    let idx = EP0_RING.enqueue;
    if idx >= EP0_RING_SIZE - 1 {
        return false;
    }
    EP0_RING.trbs[idx] = trb;
    let old = EP0_RING.trbs[idx].params[3];
    EP0_RING.trbs[idx].params[3] = (old & !1) | EP0_RING.cycle;
    EP0_RING.enqueue = idx + 1;
    true
}

/// 构造 SETUP 阶段 TRB
unsafe fn trb_setup_stage(setup: &[u8; 8], cycle: u32) -> Trb {
    let mut trb = Trb::zero();
    trb.params[0] = u32::from_le_bytes([setup[0], setup[1], setup[2], setup[3]]);
    trb.params[1] = u32::from_le_bytes([setup[4], setup[5], setup[6], setup[7]]);
    trb.params[2] = 8;
    trb.params[3] = ((TRB_TR_SETUP_STAGE as u32) << 10) | cycle | (1 << 5);
    trb
}

/// 构造 DATA 阶段 TRB
unsafe fn trb_data_stage(buf: *mut u8, len: u32, dir_in: bool, cycle: u32) -> Trb {
    let mut trb = Trb::zero();
    trb.params[0] = buf as u32;
    trb.params[1] = (buf as u64 >> 32) as u32;
    trb.params[2] = len;
    let mut p3 = ((TRB_TR_DATA_STAGE as u32) << 10) | cycle | (1 << 5);
    if dir_in {
        p3 |= 1 << 16; // DIR=IN
    }
    trb.params[3] = p3;
...
/// 构造 STATUS 阶段 TRB
unsafe fn trb_status_stage(dir_in: bool, cycle: u32) -> Trb {
    let mut trb = Trb::zero();
    let mut p3 = ((TRB_TR_STATUS_STAGE as u32) << 10) | cycle | (1 << 5);
    if !dir_in {
        p3 |= 1 << 16; // DIR=IN for status when data was OUT
    }
    trb.params[3] = p3;
    trb
}
...
/// 触发插槽门铃 (EP0 = target 1)
unsafe fn ring_doorbell(mmio: u64, caplength: u8, slot: u8) {
    let db_off = caplength as u32 + DOORBELL_BASE + (slot as u32) * 4;
    mmio_write32(mmio, db_off, 1);
}

/// 等待传输完成事件 (轮询 USBSTS.EINT)
unsafe fn wait_transfer_done(base: u64, op_offset: u32) -> bool {
    for _ in 0..500_000 {
        let usbsts = mmio_read32(base, op_offset + USBSTS);
        if usbsts & STS_EINT != 0 {
            mmio_write32(base, op_offset + USBSTS, STS_EINT);
            return true;
        }
        core::arch::asm!("pause");
    }
...
/// 向设备发出控制传输 (SETUP + 可选 DATA + STATUS)
pub unsafe fn control_transfer(
    ci: usize, slot: u8, setup: &[u8; 8], data: &mut [u8], dir_in: bool,
) -> bool {
    if ci >= unsafe { CONTROLLER_COUNT } || slot == 0 {
        return false;
    }
    ...
    EP0_RING.enqueue = 0;
    let cycle = EP0_RING.cycle;

    if !ep0_enqueue(trb_setup_stage(setup, cycle)) {
        return false;
    }

    let data_len = data.len() as u32;
    if data_len > 0 {
        if !ep0_enqueue(trb_data_stage(data.as_mut_ptr(), data_len, dir_in, cycle)) {
            return false;
        }
    }

    if !ep0_enqueue(trb_status_stage(dir_in, cycle)) {
        return false;
    }

    core::arch::asm!("mfence");
    ring_doorbell(mmio, caplength, slot);

    let done = wait_transfer_done(mmio, op_offset);
    if done {
        serial::write_str(b"xhci: control transfer complete\n");
        EP0_RING.cycle ^= 1;
    } else {
        serial::write_str(b"xhci: control transfer timeout\n");
    }
    done
}
