...
/// 派生每文件加密密钥 (HKDF-SHA256, RFC 5869)
/// master_key + salt + key_id → 64 字节 XTS 双密钥
pub fn derive_file_key(master_key: &[u8; 32], ctx: &EncryptContext) -> [u8; XTS_KEY_SIZE] {
    let mut info = [0u8; 24];
    info[..16].copy_from_slice(&ctx.salt);
    info[16..24].copy_from_slice(&ctx.key_id);

    let derived = crate::crypto::kdf::hkdf_sha256(&ctx.salt, master_key, &info, XTS_KEY_SIZE);
    let mut xts_key = [0u8; XTS_KEY_SIZE];
    xts_key.copy_from_slice(&derived[..XTS_KEY_SIZE]);
    xts_key
}
